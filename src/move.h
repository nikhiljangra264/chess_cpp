...
struct Move {
    ...
    constexpr Move(square_t _from, square_t _to, Piece _capt = Piece::EMPTY, u8 _promo = 0) :
        from(_from),
        to(_to),
        promotion_or_enpassant(_promo),
        capture(_capt) {}

    bool operator==(const Move& m) const {
        return from == m.from && to == m.to && promotion_or_enpassant == m.promotion_or_enpassant
            && capture == m.capture;
    }
...
struct board_state_t {
    ...
    constexpr board_state_t(int      _rights,
                            square_t _ep_square      = INVALID_SQ,
                            int      _halfmove_count = 0) :
        rights(_rights),
        ep_square(_ep_square),
        halfmove_count(_halfmove_count) {}

    bool operator==(const board_state_t& other) const {
        return rights == other.rights && ep_square == other.ep_square;
    }
    bool operator!=(const board_state_t& other) const { return !(*this == other); }
...
class board_history_t {
   ...
   public:
    ...
    Move get_last_move() const { return move_history.empty() ? Move() : move_history.back(); }

    board_state_t get_last_board_state() const {
        return board_state_history.empty() ? board_state_t() : board_state_history.back();
    }

    hash_t get_last_key() const { return key_history.empty() ? 0 : key_history.back(); }

    bool check_three_fold_repetition(hash_t key) const {
        return key_cache.find(key) != key_cache.end() && key_cache.at(key) >= 3;
    }
    bool position_occured(hash_t key) const { return key_cache.find(key) != key_cache.end(); }
