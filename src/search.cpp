...
score_t Search::absearch(score_t alpha, score_t beta, depth_t depth, depth_t ply) {
    ...
    std::sort(moves->begin(), moves->end(), [this, tt_move](Move& move1, Move& move2) {
        return Eval::score_move(board, move1, tt_move) > Eval::score_move(board, move2, tt_move);
    });
            ...
            if (score > alpha)
            {
                ...
                if (score >= beta)
                {
                    ...
                    if (move.capture == EMPTY)
                    {
                        auto hhBonus =
                          (depth * depth)
                          - TT::h_table[(board.side_to_move() == WHITE) ? 0 : 1][move.from.rank]
                                       [move.from.file][move.to.rank][move.to.file]
                              * abs(depth * depth) / 16384;

                        TT::h_table[(board.side_to_move() == WHITE) ? 0 : 1][move.from.rank]
                                   [move.from.file][move.to.rank][move.to.file] += hhBonus;
                    }
    ...
    auto bound = (best_score >= beta) ? TT::LOWER_BOUND
               : (alpha != oldAlpha)  ? TT::EXACT
                                      : TT::UPPER_BOUND;
    if (!check_time())
...
score_t Search::qsearch(score_t alpha, score_t beta, depth_t ply) {
    ...
    if (!TT::moves_history.has_captures(key))
    {
        ...
        temp_moves.erase(std::remove_if(temp_moves.begin(), temp_moves.end(),
                                        [](const Move& move) { return move.capture == EMPTY; }),
                         temp_moves.end());
        ...
        std::sort(temp_moves.begin(), temp_moves.end(),
                  [this](const Move& move1, const Move& move2) {
    ...
    for (auto& move : *moves)
    {
        ...
        nodes++;
        // delta pruning
        if (Eval::PieceValue[move.capture] + 400 + best_value < alpha
            && move.promotion_or_enpassant == EMPTY)
            continue;
...
std::string Search::stats(depth_t depth, score_t score, time_type time) {
    ...
    std::string info            = "info depth " + std::to_string(depth) + " score "
                     + std::move(convert_score(score)) + " nodes " + std::to_string(nodes) + " nps "
                     + std::to_string(nodes / time_in_seconds) + " time " + std::to_string(time)
                     + " pv" + std::move(getPV());
    return info;
}
