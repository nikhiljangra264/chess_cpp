...
void board_t::push(const Move& m) {
    ...
    if (board_state.ep_square != INVALID_SQ)
        key ^= Zobrist::en_passant_keys[(board_state.ep_square.rank == 2) ? 0 : 1]
                                       [board_state.ep_square.file];
    // reset en passant
    ...
    else if (is_rook(moving_piece))
    {
        ...
        if (board_state.rights)
        {
            ...
            if (turn == WHITE)
            {
                ...
                else if (m.from.file == TOTAL_FILE)
...
MOVES board_t::get_psuedo_legal_move_pawn(square_t sq) {
    ...
    else
    {
        ...
        if (!is_sq_occ(to))
        {
            ...
            to.rank += color;
            if (((color == WHITE && sq.rank == 1) || (color == BLACK && sq.rank == 6))
                && !is_sq_occ(to))
                moves.push_back(Move(sq, to));
        }
        ...
        // en passant
        if (board_state.ep_square != INVALID_SQ && board_state.ep_square.rank == sq.rank + color
            && (board_state.ep_square.file == sq.file - 1
                || board_state.ep_square.file == sq.file + 1))
            moves.push_back(
              Move(sq, board_state.ep_square, piece_at(sq.rank, board_state.ep_square.file), true));
    }
...
MOVES board_t::get_psuedo_legal_move_queen(square_t sq) const {
    ...
    rook_moves.insert(rook_moves.end(), std::make_move_iterator(bishop_moves.begin()),
                      std::make_move_iterator(bishop_moves.end()));
    return rook_moves;
}
    ...
    for (u8 rank = 0; rank < TOTAL_RANK; ++rank)
    {
        for (u8 file = 0; file < TOTAL_FILE; ++file)
        {
            ...
            else if (is_pawn(piece_at(rank, file)))
            {
                ...
                moves.insert(moves.end(), std::make_move_iterator(temp.begin()),
    ...
    for (auto& dir : ROOK_DIRECTIONS)
    {
        ...
        while (to.inbound())
        {
            ...
            if (is_sq_occ(to))
            {
                // if queen and rook
                if (piece_color(piece_at(to)) == color
                    && (is_queen(piece_at(to)) || is_rook(piece_at(to))))
                    return true;
                break;
            }
            to.rank += dir[0];
    ...
    for (auto& dir : BISHOP_DIRECTIONS)
    {
        ...
        while (to.inbound())
        {
            ...
            if (is_sq_occ(to))
            {
                // if bishop queen
                if (piece_color(piece_at(to)) == color
                    && (is_queen(piece_at(to)) || is_bishop(piece_at(to))))
                {
                    return true;
                }
                break;
...
std::ostream& operator<<(std::ostream& out, const board_t& _b) {
    ...
    out << "castling rights: " << (_b.board_state.get_wking_castle_king() ? 'K' : '-')
