...
inline int score_q_move(const board_t& board, const Move& move) {
    return (move.capture != Piece::EMPTY) ? mvvlva[move.capture % 6][board.piece_at(move.from) % 6]
                                          : 0;
}
inline int score_move(const board_t& board, const Move& move, const Move& tt_move) {
    ...
    return TT::h_table[(board.side_to_move() == WHITE) ? 0 : 1][move.from.rank][move.from.file]
                      [move.to.rank][move.to.file];
}
